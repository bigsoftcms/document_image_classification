...
def lemm_tokenize_doc(doc):
    ...
    # lemmatize, transform to ascii as will no longer use spaCy
    ...
    stop_specific = ['date', 'state', 'surface', 'location', 'oil', 'operator', 'commission', 'colorado', 'conservation', 'denver', 'ogcc', 'cogcc']
...
def lemm_tokenize_doc_spacy_pos(doc):
    ...
    noun_tokens = [unidecode(token.lemma_) for token in spacy_doc if token.pos_ == 'NOUN' or token.pos_ == 'VERB']
...
def process_corpus_spacy_pos(corpus_chunk):
    '''
    INPUT: equally sized chunks of raw corpus for pre-processing
    OUTPUT: (1) lemmatized and tokenized documents for the chunk of corpus supplied to the function.
    TASK: uses 'lemm_tokenize_doc' function to create a list of lemm-tokenized documents that correspond to all the documents in the chunk of raw corpus supplied.
    '''
    return [lemm_tokenize_doc_spacy_pos(doc) for doc in corpus_chunk]
...
def parallel_corpus_lemm_tokenization_spacy_pos(txt_paths):
    '''
    INPUT: paths to OCRd .tif files that are in .txt format.
    OUTPUT: (1) lemmatized and tokenized corpus
    TASK: use multiprocessing Pool to parallelize task using all cores on machine.
    ...
    return list(chain(*pool.map(process_corpus_spacy_pos, corpus_chunks)))
